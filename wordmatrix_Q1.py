...
def train_withoutlog(message, ls, pos_test, neg_test, pr_yi_pos_train, pr_yi_neg_train, freq_pos_train, freq_neg_train,
                     totalwords_pos_train, totalwords_neg_train, vocab):
    actual_class = []
    predicted_class = []
    for instance in pos_test:
        pos_pr = pr_yi_pos_train * pr_wk_yi_word_nolog(freq_pos_train, totalwords_pos_train, instance, vocab, ls)
        neg_pr = pr_yi_neg_train * pr_wk_yi_word_nolog(freq_neg_train, totalwords_neg_train, instance, vocab, ls)
        if pos_pr > neg_pr:
...
def naive_bayes():
    ...
    alpha = 1e-5
    message = "\nWithout log computations"
    train_withoutlog(message, alpha, pos_test, neg_test, pr_yi_pos_train, pr_yi_neg_train, freq_pos_train,
                     freq_neg_train, totalwords_pos_train, totalwords_neg_train, vocab)

    message = "\nWith log computations"
    train(message, alpha, pos_test, neg_test, pr_yi_pos_train, pr_yi_neg_train, freq_pos_train, freq_neg_train,
          totalwords_pos_train, totalwords_neg_train, vocab)
